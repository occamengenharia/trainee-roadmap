const { sumCart, addProductToCart, upperPrice, removeProductToarrayProducts, happyCart } = require('../cart');
const input = [{
        name: "produto 1",
        price: 2.25,
        quantity: 1,
...
test("Remover o primeiro elemento do carrinho", () => {
    ...
    const output = removeProductToarrayProducts(input);
...
test("Erro ao remover produto quando o carrinho está vazio", () => {
    const mock = [];
    expect(removeProductToarrayProducts(mock)).rejects.toBeInstanceOf(Error);
    console.log(expect(removeProductToarrayProducts(mock)).rejects.toBeInstanceOf(Error));
});
